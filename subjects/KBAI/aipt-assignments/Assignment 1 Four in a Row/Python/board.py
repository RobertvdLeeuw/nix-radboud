...
if __name__ == "__main__":
    start_board = Board(7, 6)
    root = create_tree(start_board, player=1, depth=2)

    print("Number of children:", len(root.children))
    for i, child in enumerate(root.children):
        print(f"Child {i} (leaf: {child.leaf})")
        print(child.state)

    root2 = create_tree(Board(7, 6), player=1, depth=2)
    total = 0
    for i, child in enumerate(root2.children):
        print(f"Number of grandchildren: {len(child.children)}")
        for j, grandchild in enumerate(child.children):
            grid = grandchild.state.get_board_state()
            ones = (grid == 1).sum()
            twos = (grid == 2).sum()
            print(f"  Grandchild {j}: player1={ones}, player2={twos}, leaf={grandchild.leaf}")
            print(grandchild.state)
            total += 1

    print("Total grandchildren:", total)
