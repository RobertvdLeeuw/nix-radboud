...
def get_players(game_n: int) -> List[PlayerController]:
    ...
    minimax1: MinMaxPlayer = MinMaxPlayer(2, game_n, 4, heuristic2)
    minimaxalphabeta1: AlphaBetaPlayer = AlphaBetaPlayer(1, game_n, 4, heuristic1)

    # TODO: Implement other PlayerControllers (MinMaxPlayer and AlphaBetaPlayer)

    players: List[PlayerController] = [minimax1, minimaxalphabeta1]
    ...
    assert players[1].player_id in {1, 2}, (
        "The player_id of the second player must be either 1 or 2"
    )
    assert players[0].player_id != players[1].player_id, "The players must have an unique player_id"
    assert players[0].heuristic is not players[1].heuristic, (
        "The players must have an unique heuristic"
    )
